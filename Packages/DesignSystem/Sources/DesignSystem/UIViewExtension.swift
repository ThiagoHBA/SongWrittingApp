...
extension UIView {
    public var allTemplateViews: [UIView] {
        var views = [UIView]()
...
extension UIView {
    public func setShimmeringAnimation(_ animate: Bool, viewBackgroundColor: UIColor? = nil) {
...
extension UIView {
    public func setTemplate(_ template: Bool, baseColor: UIColor? = nil) {
        var color: UIColor
        if let baseColor = baseColor {
            color = baseColor
        } else {
            if #available(iOS 12, *), traitCollection.userInterfaceStyle == .dark {
                color = SWColor.Placeholder.dark
            } else {
                color = SWColor.Placeholder.light
            }
        }
